...
export default function BlackHole() {
  ...
  useEffect(() => {
    ...
    const offsetX = isMobile ? 0 : mount.clientWidth * -0.25;
    camera.setViewOffset(mount.clientWidth, mount.clientHeight, offsetX, 0, mount.clientWidth, mount.clientHeight);
    ...
    // Start the disk at a steep angle (edge-on) to create a faint profile
    accretionDisk.rotation.set(2.48, 0, Math.PI / 4);
    ...
    const resize = () => {
        ...
        const isMobile = window.innerWidth <= 900;
        const offsetX = isMobile ? 0 : w * -0.25;
    ...
    // Setup GSAP ScrollTrigger for the Grand Rotation Reveal
    gsap.registerPlugin(ScrollTrigger);
    
    const revealAnim = gsap.to(accretionDisk.rotation, {
        x: DISK_TILT_ANGLE,
        y: 0,
        z: 0,
        duration: 4.0,
        ...
        scrollTrigger: {
            trigger: mount,
            start: "top 75%", // Trigger when the top of the black hole enters 75% down the screen
        }
    });
